    ...
    chrome.storage.local.get(
      ...
      (result: { userPreferences?: IUserPreferences }) => {
        if (
          ...
          result.userPreferences.automationSettings
        ) {
          const settings: AutomationSettings =
            result.userPreferences.automationSettings;
          const automationToggle = document.getElementById(
            "automation-toggle"
          ) as HTMLInputElement | null;
          const automationSettingsElem = document.getElementById(
            "automation-settings"
          );

          if (automationToggle && automationSettingsElem) {
            automationToggle.checked = settings.enabled;
            automationSettingsElem.classList.toggle(
              "hidden",
              !settings.enabled
            );
          }
          ...
          if (settings.templates) {
            const highTemplateElem = document.getElementById(
              "high-template"
            ) as HTMLTextAreaElement | null;
            if (highTemplateElem)
              highTemplateElem.value = settings.templates.high || "";
          }
        }
      }
    );
