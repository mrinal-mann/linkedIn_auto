...
export class MessageManager {
  ...
  public extractMessages(): Message[] {
    ...
    if (window.location.href.includes("messaging")) {
      const conversationItems: NodeListOf<Element> = document.querySelectorAll(
      ...
      if (conversationItems.length > 0) {
        ...
        conversationItems.forEach((item: Element) => {
          const senderElement: Element | null = item.querySelector(
            ".msg-conversation-card__participant-names"
          );
          const sender: string = senderElement
            ? senderElement.textContent?.trim() || "Unknown"
            : "Unknown";
          const previewElement: Element | null = item.querySelector(
            ".msg-conversation-card__message-snippet"
          );
          const preview: string = previewElement
            ? previewElement.textContent?.trim() || ""
            : "";
          const timeElement: Element | null = item.querySelector(
            ".msg-conversation-card__time-stamp"
          );
          const timestamp: string = timeElement
  ...
  public checkForNewMessages(): void {
    ...
    if (window.location.href.includes("messaging")) {
      ...
      if (newOrUpdatedMessages) {
        console.log(
          `Updated message collection: ${this.messages.length} messages`
        );
        this.saveMessagesToStorage();
      }
    }
